...
const ProjectModal: React.SFC<IProps> = ({ currentProject }) => {
    ...
    const { name, date, image, alt, caption, site, repo, desc } = projectData[
        currentProject
    ];
    ...
    const toggleImage = (e: mouseEvent) => {
        if (
            !(e.target instanceof HTMLElement) ||
            (isImageVisible && e.target.dataset.util !== "image-close")
        ) {
            return;
        }
    ...
    const closeModal = (e: mouseEvent) => {
        if (
            !(e.target instanceof HTMLElement) ||
            e.target.dataset.util !== "modal-close" ||
            isImageVisible
        ) {
            return;
        }
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            currentArticle.removeEventListener(
                "scroll",
        ...
        >
            <div className="main">
                <div className="content">
                    ...
                    <article
                        tabIndex={1}
                        className={`modal-article ${scrollState}`}
                        ref={article}
                    >
                        ...
                        <div className="article-content">
                            ...
                            <div className="image">
                                <img
                                    onFocus={() =>
                                        imageElement.current!.addEventListener(
                                            "keydown",
                                            handleImageKeyPress
                                        )
                                    }
